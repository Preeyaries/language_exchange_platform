...
// Reason: API instance is shared across all components.
// MapComponent uses OpenStreetMap via Leaflet — public API, no API key.
...
// Reason: MapComponent hides Leaflet initialization, geocoding,
// and cleanup behind a simple city/country component.
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      tryGeocode(`${city}, ${country}`)
        .then((data) =>
          !data || data.length === 0 ? tryGeocode(country) : data
        )
        .then((data) => {
          if (cancelled || !data || data.length === 0 || !mapRef.current) {
            return;
          }
      ...
      {[1, 2, 3, 4, 5].map((i) => (
        <div
          ...
          className={`w-[7px] h-[7px] rounded-full ${
            i <= filled ? (green ? "bg-green-400" : "bg-white") : "bg-white/25"
          }`}
        />
      ))}
    </div>
...
export default function Profile() {
  ...
  const isOwn = !id || id === me?.id || id === me?._id;
  ...
  const fetchProfile = async () => {
    ...
    try {
      ...
      try {
        ...
        // Keep profile loaded even if follow stats fail
      }
    } catch {
  ...
  const userId = user?._id || user?.id || me?.id || me?._id || "default";
  ...
  return (
    <PhoneFrame>
      <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        {/* Map header */}
        <div
          className="relative h-[200px] overflow-hidden bg-[#1a3575]"
          style={{ zIndex: 0 }}
        >
          {city && country ? (
            <div
          ...
          <div
            className="absolute inset-0 bg-gradient-to-t from-[#0f1c3f]/80 via-[#0f1c3f]/10 to-transparent pointer-events-none"
            style={{ zIndex: 10 }}
          />

          {city && country && (
            <div
              className="absolute bottom-5 right-4 text-right"
              style={{ zIndex: 20 }}
            >
              <div className="text-white text-sm font-extrabold drop-shadow-lg">
                {city}
              </div>
          ...
          {isOwn && (
            <Link
              to="/profile/edit"
              style={{ zIndex: 500, position: "absolute", top: 16, right: 16 }}
            >
        ...
        {/* Profile card */}
        <div
          className="bg-gradient-to-b from-[#1a2d6b] to-[#0f1c3f] rounded-t-[28px] relative px-8 pb-24 animate-[fadeUp_0.4s_ease_both] min-h-screen"
        ...
        >
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex-[2] text-center">
              ...
              <div className="text-white/45 text-[13px] font-semibold">
                {handle}
              </div>
            </div>
          ...
          {bio && (
            <p className="text-center text-white/70 text-[13px] font-semibold leading-snug mt-2.5 mb-4 px-2">
              {bio}
            </p>
          )}
          ...
          <div className="flex items-center justify-center gap-4 mb-5 flex-wrap">
            ...
            {learningLangs.length > 0 && (
              <span className="text-white/40 text-lg mb-3">⇌</span>
            )}
            {learningLangs.map((l, i) => (
              <LangItem key={i} lang={l.language} level={l.level} />
            ))}
          </div>
          ...
          {!isOwn && (
            <div className="flex gap-3 mb-6">
              <button
                ...
                className={`flex-1 rounded-3xl py-3.5 text-white text-[15px] font-extrabold cursor-pointer transition-all disabled:opacity-60 disabled:cursor-not-allowed ${
                  isFollowing
                    ? "bg-white/10 border border-white/20"
                    : "bg-[#4a7fe0] border-0 shadow-[0_4px_16px_rgba(74,127,224,0.4)] hover:bg-[#5a8ff0]"
                }`}
              >
          ...
          {interests.length > 0 && (
            <>
              ...
              <p className="text-white text-[15px] font-black mb-3">
                Personal Interest
              </p>
              <div className="flex flex-wrap gap-2">
                {interests.map((item) => (
                  <span
                    key={item}
                    className="bg-white/10 rounded-full px-3.5 py-1.5 text-white/70 text-xs font-bold"
                  >
                    {item}
                  </span>
                ))}
